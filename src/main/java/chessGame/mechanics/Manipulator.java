 ...
 */
public class Manipulator extends Application {
    ...
    public static class Arrow extends Polygon {
         ...
         * @return a Point2D
         */
        private Point2D eval(CubicCurve c) {
            return new Point2D(Math.pow(1, 3) * c.getStartX() +
                    3 *Math.pow(1, 2) * c.getControlX1() +
                    3* c.getControlX2() +
                    Math.pow(1, 3) * c.getEndX(),
                    c.getStartY() +
                            3 * Math.pow(1, 2) * c.getControlY1() +
                            3 * c.getControlY2() +
                            c.getEndY());
        }
        ...
        private Point2D evalDt(CubicCurve c, float t) {
            ...
            x = x + 3 * (Math.pow(1 - t, 2) - 2 * t * (1 - t)) * c.getControlX1();
            x = x + 3 * ((1 - t) * 2 * t - t * t) * c.getControlX2();
            x = x + 3 * Math.pow(t, 2) * c.getEndX();

            double y = -3 * Math.pow(1 - t, 2) * c.getStartY();
            y = y + 3 * (Math.pow(1 - t, 2) - 2 * t * (1 - t)) * c.getControlY1();
            y = y + 3 * ((1 - t) * 2 * t - t * t) * c.getControlY2();
            y = y + 3 * Math.pow(t, 2) * c.getEndY();
            return new Point2D(x, y);
        }

        private Point2D evalSingle(CubicCurve c) {
           return new Point2D(c.getStartX()- c.getControlX1(), c.getStartY() - c.getControlY1()).normalize();
        }
    }
